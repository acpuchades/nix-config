...
| rPackages.V8 icu78 force-link | `modules/r-dev/system.nix` (`nixpkgs.overlays`) | Drop once nixpkgs realigns ICU (nodejs-libv8 built with ICU 78 vs system ICU 76) or ships a working `r-V8`. | **Keep (added 2026-07-30)** — `gt`→`juicyjuice`→`V8` failed with undefined `icu_78::*` symbols; overlay links `icu78` (icu4c-78.3) into V8 via the r-modules `overrides` hook so `gt`/`gtsummary` build. |
