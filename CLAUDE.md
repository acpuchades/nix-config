...
These carry ongoing bookkeeping tied to external state. On each `nix flake update` (or every few months), re-check each one and remove it once its drop condition is met. Last reviewed: 2026-08-28.
...
| fugazi-web sub-hourly tick sizing | `modules/fugazi-web/default.nix` (`cadenceSeconds`/`tickTimeout` + the `systemd.services` sub-hourly branch) | Drop when the IMPORTED input (`fugazi-web`, the `main` branch) carries upstream's own per-cadence tick sizing — its `testing` branch already does. The tell on a rebuild: a tick unit that arrives with a `TimeoutStartSec` shorter than the flat `15m`, at which point our `mkForce` starts overriding a considered value. Re-check on each `nix flake update fugazi-web`. | **Keep (added 2026-08-24)** — the testing instance runs `testing`-branch packages under `main`-branch plumbing (see the `packages` option header), and `main` gives every tick unit one flat `TimeoutStartSec=15m` — the whole period at `15m`, where a slow tick outlives its slot (systemd skips the next run, bars silently untraded) and its SIGTERM lands mid-batch with no JSON line. Shim sets `TimeoutStartSec` to min(15m, 90% of the cadence) and `FUGAZI_SERVICE_DEPLOYMENT_TICK_BUDGET_SECONDS` to 85% of that, copying upstream's arithmetic — 270s/229 at `5m`, 810s/688 at `15m`, 900s/765 at `30m`. Only applies to frequencies under an hour; hourly and coarser keep upstream's flat 15m. |
