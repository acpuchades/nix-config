;; Core user defaults. Moved here from init.el's body: everything in
;; ~/.emacs.d/config/ loads in filename order, so settings that lived in
;; init.el itself ran AFTER 99-personal.el and silently overrode it —
;; "99 loads last" is only true now that init.el is a bare loader.

;; Start emacs server (not under the systemd daemon, which is its own server)
(require 'server)
