# projects skill generator: the working CONVENTION for Python/R project repos
# (distinct from policy's security BOUNDARY and toolkit's capability INVENTORY).
# The agent's own interpreters are a fixed Nix closure, which is right for scratch
# work and wrong inside a project that declares its own environment — this skill
# draws that line so it doesn't bootstrap a venv for a five-line script, or
# conversely pollute a project by installing into its own baked library.
{ pkgs, lib, icfg, homeDir, ... }:

pkgs.writeTextDir "projects/SKILL.md" ''
        ---
        name: projects
        ...
        **R — use `rix`** to generate a pinned Nix environment, then let direnv load it:
        ...
        Keep clones under your own tree (`${homeDir}`). Remember that `git pull` and
        every other remote verb follows the normal approval rules in the `policy`
        skill.
      ''
